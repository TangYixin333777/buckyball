# Buckyball Conductor Im2col E2E Promote V1

> Superseded by `BUCKYBALL_CONDUCTOR_IM2COL_E2E_V2.prompt.md`.
> Keep this V1 file only for backward compatibility.

Use this promote when you want `buckyball-conductor` to run a fully automated end-to-end im2col workflow with strict debug/optimize separation and contract outputs.
...
3) 仅当该轮再次得到 strict_pass，才允许调用 Yosys/OpenSTA评估该轮面积/时序收益；
4) 若该轮不是 strict_pass，则回到 debug-only，不得运行该轮综合评估。

五、执行流程

先执行 requirement clarification，输出精简 Requirement Plan：
operator_name, matrix_shape, data_type, isa_policy, test_vector_policy, runtime_policy, assumptions
...
DEBUG_OPTIMIZE_RESULT_V1（必须包含 baseline/post/comparison/validate）
并追加每一轮优化收益汇总（每轮面积下降、开销下降、延时下降/Slack改善，以及总计改善）
```
